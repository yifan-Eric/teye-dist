...
class ReactCanvasNest extends Component {
    ...
    componentDidMount = () => {
        ...
        if (!style.position || style.positon === 'static') { style.position = 'relative'; }
    ...
    randomPoints = () => {
        ...
        for (let index = 0; index < count; ++index) {
            points.push({
                ...
                xStep: Math.random() - 0.5, // step size of point movement
    ...
    init = () => {
        ...
        this.setState({
            ...
            window.requestAnimationFrame(this.drawNest);
        });
    }
        ...
        for (let index = 0; index < points.length; ++index) {
            ...
            for (let nextIndex = 0; nextIndex < pointsWithMouse.length; ++nextIndex) {
                ...
                if (nextPoint.x) {
                    ...
                    if (dist < nextPoint.max) { if (nextIndex + 1 === pointsWithMouse.length && dist >= nextPoint.max / 2) { point.x -= 0.03 * xDist; point.y -= 0.03 * yDist; }; }
